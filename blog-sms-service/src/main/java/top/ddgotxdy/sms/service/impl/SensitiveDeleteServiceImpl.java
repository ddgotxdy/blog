...
public class SensitiveDeleteServiceImpl extends AbstractSmsService {
    ...
    protected boolean filter(SmsContext smsContext) {
        ...
        if (!allCommonCheck) {
            throw new BlogException(ResultCode.SENSITIVE_DELETE_ERROR.getCode(), "not admin");
        }
        // 2. 传递的列表
        if (Objects.isNull(smsContext.getSensitiveIds())) {
            throw new BlogException(ResultCode.SENSITIVE_DELETE_ERROR.getCode(), "Sensitive Ids empty");
        }
        return true;
    }
