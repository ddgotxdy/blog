 ...
 * @description:
 */
@SmsEventSelector(SmsEvent.COMMENT_DELETED)
@Service
@Slf4j
public class CommentDeleteServiceImpl extends AbstractSmsService {
    @Resource
    ...
    protected boolean filter(SmsContext smsContext) {
        ...
        if (Objects.isNull(userId)) {
            throw new BlogException(ResultCode.COMMENT_DELETE_ERROR.getCode(), "用户id为空");
        }
        // 2. comment ids 校验
        List<Long> commentIds = smsContext.getCommentIds();
        if (CollectionUtils.isEmpty(commentIds)) {
            throw new BlogException(ResultCode.COMMENT_DELETE_ERROR.getCode(), "评论ids为空");
        }
        return true;
    }
    ...
    protected void doExecute(SmsContext smsContext) {
        List<Long> commentIds = smsContext.getCommentIds();
        boolean deleteOk = blogCommentService.deleteBatchByIds(commentIds);
        // 没有被删除的
        if (deleteOk) {
