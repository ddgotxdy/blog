 ...
 * @description:
 */
@SmsEventSelector(SmsEvent.SENSITIVE_RECOVER)
@Service
@Slf4j
public class SensitiveRecoverServiceImpl extends AbstractSmsService {
    @Resource
    ...
    protected void doExecute(SmsContext smsContext) {
        ...
        boolean ok = blogSensitiveService.recoverBatchByIds(sensitiveIds);
        if (ok) {
