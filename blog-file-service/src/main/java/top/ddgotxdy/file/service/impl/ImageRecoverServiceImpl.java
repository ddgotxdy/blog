 ...
 * @description:
 */
@FileEventSelector(FileEvent.IMAGE_RECOVER)
@Service
@Slf4j
public class ImageRecoverServiceImpl extends AbstractFileService {
    @Resource
    ...
    protected boolean filter(FileContext fileContext) {
        ...
        if (Objects.isNull(userId)) {
            throw new BlogException(ResultCode.IMAGE_RECOVER_ERROR.getCode(), "用户id为空");
        }
        // 2. image id 列表是否传过来
        List<Long> imageIds = fileContext.getImageIds();
        if (CollectionUtils.isEmpty(imageIds)) {
            throw new BlogException(ResultCode.IMAGE_RECOVER_ERROR.getCode(), "image id列表为空");
        }
        return true;
    }
    ...
    protected void doExecute(FileContext fileContext) {
        List<Long> imageIds = fileContext.getImageIds();
        boolean ok = blogImageService.recoverByImageIdList(imageIds);
        if (ok) {
